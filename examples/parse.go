...
Alice,28,62,000,2019-11-20,false
Bob,35,70,000,2021-03-10,true`

	csvReader := strings.NewReader(csvData)
	csvTypes, err := InferCSVFileTypes(csvReader)
	if err != nil {
		fmt.Println("Error reading CSV:", err)
		return
	}
	fmt.Println("CSV Column Types:")
	for k, v := range csvTypes {
		fmt.Printf("  %s: %s\n", k, v)
	}

	// --- JSON Example ---
	// The JSON data here is an array of objects.
	jsonData := `[{
		"Name": "John",
		"Age": 30,
		"Salary": 55000.50,
	...
	},
	{
		...
		"Age": 28,
		"Salary": 62000,
		"StartDate": "2019-11-20T00:00:00Z",
		"Active": false,
	...
	jsonFieldTypes, err := InferJSONFileType(jsonObj)
	if err != nil {
		fmt.Println("Error inferring JSON field types:", err)
		return
	}
	fmt.Println("\nJSON Field Types:")
	for k, v := range jsonFieldTypes {
		fmt.Printf("  %s: %s\n", k, v)
	}
}
